...
export default function Simulator() {
  ...
  const pull = (v) => {
    if (!v) return
    if (state.mSim.items.some((x) => x.src === v)) return toast(t('Already added'), 'error')
    let it = null
    if (v === 'debt-all') it = { desc: t('All debt minimums'), amount: mins }
      ...
      it = { desc: t('My avg monthly spending (everything)'), amount: Math.round(avg) }
    } else if (v.startsWith('d:')) {
      const d = state.debts[+v.slice(2)]
      if (d) it = { desc: t('{name} (min payment)', { name: d.name }), amount: d.min }
    } else if (v.startsWith('r:')) {
      const r = state.recurring.find((x) => x.id === v.slice(2))
      if (r) it = { desc: recEvery(r) > 1 ? t('{name} (monthly avg)', { name: r.desc }) : r.desc, amount: +recMonthly(r).toFixed(2) }
    }
    if (it) update((s) => { s.mSim.items.push({ id: uid('ms'), src: v, ...it }) })
  }

  const addCustom = () => {
    if (!name.trim()) return toast(t('Name the expense'), 'error')
    const a = parseFloat(amt)
    if (isNaN(a)) return toast(t('Enter an amount'), 'error')
    ...
    <div className="fade-in space-y-6">
      <Card className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <SectionHead title={t('Plan a month')} desc={t("Type an income, stack up expenses, see what's left. Pretend mode — nothing here touches your real data.")} />
          <Button variant="outline" size="sm" onClick={() => update((s) => { s.mSim = { income: '', items: [] } })}><RotateCcw />{t('Start over')}</Button>
        </div>
        <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
          <div className="rounded-xl border border-emerald-400/15 bg-emerald-400/[0.06] px-2 py-3 text-center sm:p-4">
            <div className="mb-1 text-[0.625rem] uppercase tracking-wider text-muted-foreground sm:text-[0.6875rem]">{t('Income')}</div>
          ...
          <div className="rounded-xl border border-red-400/15 bg-red-400/[0.05] px-2 py-3 text-center sm:p-4">
            <div className="mb-1 text-[0.625rem] uppercase tracking-wider text-muted-foreground sm:text-[0.6875rem]">{t('Expenses')}</div>
          ...
          <div className={`rounded-xl border px-2 py-3 text-center sm:p-4 ${net >= 0 ? 'border-emerald-400/25 bg-emerald-400/[0.06]' : 'border-red-400/30 bg-red-400/[0.08]'}`}>
            <div className="mb-1 text-[0.625rem] uppercase tracking-wider text-muted-foreground sm:text-[0.6875rem]">{net >= 0 ? t('Left Over') : t('In The Negative')}</div>
        ...
        {income > 0 && (
          <div className="mt-3">
            <div className="mb-1.5 flex justify-between text-[0.6875rem] text-muted-foreground">
              <span>{t('Spent')}</span><span className={pct >= 100 ? 'font-semibold text-red-400' : ''}>{t('{pct}% of income', { pct })}</span>
            </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card className="p-5">
          <SectionHead title={t('1 · Income for the month')} />
          <div className="mt-3 flex flex-wrap items-end gap-2">
            ...
            {avgInc > 0 && <Button variant="outline" size="sm" onClick={() => setIncome(Math.round(avgInc))}>{t('My avg ({amount})', { amount: fmt0(avgInc) })}</Button>}
            <Button variant="outline" size="sm" onClick={() => setIncome(Math.round(incomeIn(state, today().slice(0, 7))))}>{t('This month so far')}</Button>
          </div>
        </Card>
        <Card className="p-5">
          <SectionHead title={t('2 · Add expenses')} />
          <div className="mb-3 mt-3">
            <Label>{t('Pull from what I already have')}</Label>
            <Select className="w-full" value="" onChange={(e) => pull(e.target.value)}>
              <option value="">{t('Choose to add…')}</option>
              <option value="debt-all">{t('All debt minimums — {amount}/mo', { amount: fmt0(mins) })}</option>
              <option value="rec-all">{t('All recurring bills — {amount}/mo', { amount: fmt0(recT) })}</option>
              <option value="spend-avg">{t('My avg monthly spending (everything)')}</option>
              <optgroup label={t('Single debts')}>
                {state.debts.map((d, i) => <option key={d.name} value={`d:${i}`}>{t('{name} — {amount}/mo', { name: d.name, amount: fmt0(d.min) })}</option>)}
              </optgroup>
              <optgroup label={t('Single recurring bills')}>
                {state.recurring.filter((r) => r.active !== false).map((r) => <option key={r.id} value={`r:${r.id}`}>{t('{name} — {amount}/mo', { name: r.desc, amount: fmt0(recMonthly(r)) })}</option>)}
              </optgroup>
            </Select>
          </div>
          <Label>{t('Or type your own')}</Label>
          <div className="flex gap-2">
            <Input className="flex-1" placeholder={t('e.g. Groceries')} value={name} onChange={(e) => setName(e.target.value)} />
            <Input type="number" step="10" min="0" className="w-28" placeholder="$" value={amt} onChange={(e) => setAmt(e.target.value)} />
            <Button className="shrink-0" onClick={addCustom}><Plus />{t('Add')}</Button>
          </div>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center border-b bg-secondary/40 px-4 py-2.5 text-[0.65625rem] font-semibold uppercase tracking-wider text-muted-foreground">
          <span>{t('Expense list')}</span><span className="ml-auto">{t('{n} item{plural} · {amount}', { n: ms.items.length, plural: ms.items.length === 1 ? '' : 's', amount: fmt0(totalExp) })}</span>
        </div>
        <div className="divide-y divide-border/60">
          {ms.items.length ? ms.items.map((x) => (
            <div key={x.id} className="flex items-center gap-3 px-4 py-2">
              ...
              {x.src !== 'custom' && <Badge>{t('pulled')}</Badge>}
          ...
          )) : <div className="p-10 text-center text-[0.8125rem] text-muted-foreground">{t('No expenses yet — pull some in above or type your own.')}</div>}
        </div>
      </Card>
    </div>
  )
